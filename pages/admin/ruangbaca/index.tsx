...
// import RekapCard from "@/components/Admin/Berita/RekapCard"; // Bisa digunakan kembali

// Impor data dan tipe dari Prisma (setelah Anda fetch)
import { reading_materials } from "@prisma/client"; // Asumsi nama model dari Prisma

// --- Tipe Data Lokal (jika perlu penyesuaian dari tipe Prisma) ---
type MateriDenganUploader = reading_materials & {
  uploader?: { nama_lengkap: string | null } | null;
};

// === KOMPONEN-KOMPONEN PEMBANTU (Lokal atau Impor) ===

// 1. Komponen Kartu Rekap (Bisa diimpor dari file terpisah)
...
// === KOMPONEN UTAMA HALAMAN MANAJEMEN RUANG BACA ===
const AdminRuangBacaPage: NextPageWithLayout = () => {
  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        ...
        const response = await fetch("/api/ruang-baca");
  ...
  // Daftar kategori unik untuk tab filter
  ...
  const handleEdit = (id: bigint) => {
    router.push(`/admin/ruang-baca/edit/${id}`);
  };
  const handleDelete = useCallback(async (id: bigint) => {
    // TODO: Implement logic to delete materi by id
    console.log("Delete materi with id:", id);
    /* ... Logika fetch API DELETE ... */
  }, []);
  // Anda bisa tambahkan handler lain seperti handleApprove jika ada alur persetujuan
    ...
    <div className="space-y-8">
      ...
      <div className="flex flex-col sm:flex-row justify-between sm:items-center gap-4">
        <div>
          ...
          <p className="text-sm text-text-secondary mt-1">
            Kelola semua materi, modul, dan panduan yang tersedia untuk
            pengguna.
          </p>
        </div>
      ...
      <section>
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4">
          ...
          />
          {/* Tambahkan kartu rekap lain jika perlu */}
        </div>
      </section>
      ...
      <div className="bg-surface-card p-4 sm:p-6 rounded-xl shadow-md">
        ...
        <div className="mt-6">
          {/* Komponen Paginasi akan ditempatkan di sini */}
        </div>
      </div>
    </div>
  );
